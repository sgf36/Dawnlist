...
RULES: list[tuple[str, str, str]] = [
    (r"DL-XXXX-XXXX-XXXX-XXXX",
     "DL-XXXX-XXXX-XXXX-XXXX",
     "LIVE review code — 25 activations of a paid plan against real metered "
     "feed credits, and people scan public GitHub for key-shaped strings. "
     "Also present in HISTORY, which a scrub of the working tree cannot fix."),
    (r"ntfset_REDACTED",
     "ntfset_REDACTED",
     "Easy-Post's webhook destination id, which leaked into this repository's "
     "runbook. Another product's infrastructure that does not belong here."),
]
...
def main() -> int:
    ...
    for rel, path in tracked_files():
        ...
        for pattern, repl, why in RULES:
            ...
            if n:
                findings[(rel, why)] = findings.get((rel, why), 0) + n
